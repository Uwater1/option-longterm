...
Compute robust marginal association per feature (Spearman rank correlation) between each of the 238 features and the target. Apply BH-FDR correction across the tests. Keep the surviving set (FDR = 0.20).

**Step 1.5 — Hierarchical Feature Clustering.**
Cluster surviving features into groups based on correlation distance (threshold = 0.7 complete correlation distance). From each cluster, keep only the feature with the highest absolute Spearman correlation with the target. This reduces features to about 20-40 non-collinear candidates, stabilizing stability selection.

**Step 2 — Stability selection on survivors.**
Run repeated subsampling ($B=100$, subsample size $\lfloor N/2 \rfloor$) over the cluster survivors. Fit Lasso paths and keep features selected in $\ge 0.60$ fraction of subsamples (fallback to top 5 if count < 3).
