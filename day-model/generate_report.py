...
def _write_report(results_dict):
    ...
    lines.append("Side-aware objective: `single` (legacy two-sided Tail IC), `long` "
                 "(top-only, `pred >= P90(pred)`), `short` (bot-only, `pred <= P10(pred)`). "
    ...
    lines.append("   - `long`: Tail IC top-only (`pred >= P90(pred)`). Weights [0.45, 0.45, 0.10, 0.00] (V4 dropped).")
    lines.append("   - `short`: Tail IC bot-only (`pred <= P10(pred)`). Weights [0.45, 0.45, 0.10, 0.00] (V4 dropped).")
