...
# ─────────────────────────────────────────────────────────────────────────────
def train_statsmodels_qr(X: pd.DataFrame, y: pd.Series, features: list):
...
# ─────────────────────────────────────────────────────────────────────────────
def train_lightgbm_qr(X: pd.DataFrame, y: pd.Series, features: list):
    """Train LightGBM Quantile model at q=0.10. Returns fitted model."""
...
def rolling_validation(X: pd.DataFrame, y: pd.Series, features: list,
    ...
    for i, start_idx in enumerate(retrain_indices):
        ...
        if model_type in ("lightgbm", "ensemble"):
            trained_model = train_lightgbm_qr(train_X, train_y, features)
        ...
        # Predict
        preds_lgb = predict_lightgbm_qr(trained_model, test_X, features) if trained_model else None
        preds_sm  = predict_statsmodels_qr(trained_sm_result, test_X, features) if trained_sm_result else None

        if model_type == "ensemble" and preds_lgb is not None and preds_sm is not None:
            preds = (preds_lgb + preds_sm) / 2.0
        elif model_type == "lightgbm" and preds_lgb is not None:
            preds = preds_lgb
...
def save_model(features, model_type, etf_key, sm_coeffs=None, lgb_model=None,
    ...
    """Save model metadata + LightGBM model to disk for backtest integration."""
    os.makedirs(OUT_DIR, exist_ok=True)

    # Save LightGBM model file if available
    lgb_path = os.path.join(OUT_DIR, f"open_high_lgb_{etf_key}.txt")
...
def load_model(etf_key: str) -> dict:
    ...
    lgb_model = None
    if meta.get("lgb_model_path") and os.path.exists(meta["lgb_model_path"]):
        lgb_model = lgb.Booster(model_file=meta["lgb_model_path"])

    meta["lgb_booster"] = lgb_model
    return meta


def predict_single(model_meta: dict, features_df: pd.DataFrame) -> float:
    """Predict p10 offset for a single row (latest data point)."""
    features = model_meta["features"]
    model_type = model_meta["model_type"]

    preds = []
    if model_type in ("lightgbm", "ensemble") and model_meta.get("lgb_booster"):
        pred_lgb = model_meta["lgb_booster"].predict(features_df[features].values)[0]
...
def main():
    ...
    parser.add_argument("--max-features", type=int, default=4,
                        help="Maximum number of features to select (default: 4)")
    ...
    if args.predict:
        ...
        p10 = predict_single(meta, latest)
    ...
    y = df["y"].copy()
    print(f"  Data: {len(df)} trading days, {len(feature_cols)} candidate features")
    ...
    lgb_model = train_lightgbm_qr(X, y, best_features)
    print("  LightGBM trained.")

    sm_coeffs = None
    try:
        sm_result = train_statsmodels_qr(X, y, best_features)
    ...
    print(f"\nStep 5: Rolling validation (retrain every {args.retrain_every} days)...")
    roll_result = rolling_validation(X, y, best_features, model_type,
                                      retrain_every=args.retrain_every)
    print(f"  Predictions: {roll_result['n_predictions']} days")
    print(f"  Coverage: {roll_result['coverage']:.1f}% (target: ~90%)")
    print(f"  Mean predicted offset: {roll_result['mean_pred_offset']:.4f}%")
    print(f"  Mean actual: {roll_result['mean_actual']:.4f}%")
    print(f"  Pinball loss: {roll_result['pinball_loss']:.6f}")

    # Step 6: Static baseline comparison
    print(f"\nStep 6: Static baseline comparison...")
    static_p10 = compute_static_baseline(X, y)
    ...
    print(f"  Model coverage: {roll_result['coverage']:.1f}%")
    print(f"  Model mean offset: {roll_result['mean_pred_offset']:.4f}%")

    improvement = roll_result["mean_pred_offset"] - static_mean
    ...
    # Step 7: Feature importance from LightGBM
    lgb_importance = dict(zip(best_features, lgb_model.feature_importances_))
    ...
    print(f"  Coverage: {roll_result['coverage']:.1f}%")
