...
- **Audit log contract**: qmt_strategy writes machine-diffable `AUDIT` lines (per-feature raw values, per-feature signed z, composite, thresholds, side, entry/stop/exit events) to `qmt_audit_log.txt`. Every line ends with `TIME=HH:MM:SS` (wall clock): the DECISION TIME monitors decision latency (must be ~10:00:00); it is stripped before diffing since replay stamps its post-close run time. Exactly one DECISION per ETF per day is enforced by scanning the audit file itself (restart/re-init safe). After market close, `QMT_short/qmt_audit_replay.py` recomputes the same decision path from downloaded repo data (no QMT) and emits identical `AUDIT` lines; diffing isolates any QMT-side data/behavior divergence (the differing key names the problem). Replay also adds `REPLAY_XCHECK` vs `features_{ETF}.parquet`.
