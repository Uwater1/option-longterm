...
def main():
    ...
    df['vol20'] = df[close_col].pct_change().rolling(20).std() * np.sqrt(252)
    df['roc10'] = ta.roc(df[close_col], length=10)
    df['roc5'] = ta.roc(df[close_col], length=5)
    df['sma50'] = ta.sma(df[close_col], length=50)
    bbands_lower = ta.bbands(df[close_col], length=20, std=2)
    if bbands_lower is not None:
        df['bb_lower'] = bbands_lower.iloc[:, 0]
    df['bb_width'] = (df['bb_upper'] - df['bb_lower']) / df['sma20']
    df['atr10'] = ta.atr(df[high_col], df[low_col], df[close_col], length=10)
    cci = ta.cci(df[high_col], df[low_col], df[close_col], length=20)
    if cci is not None:
        df['cci20'] = cci
    df['vol_ratio'] = df['atr20'] / df['atr10']
    df['tr'] = ta.true_range(df[high_col], df[low_col], df[close_col])
