...
- **Multicollinearity & Complexity Control**: Controls multivariate collinearity via iterative VIF pruning (threshold <= 10.0) on stable representatives. Overfitting is prevented via a dynamic active feature cap tied to Effective Sample Size: active_features <= ESS / 15. The ESS constraint is softened into the objective as a continuous penalty.
