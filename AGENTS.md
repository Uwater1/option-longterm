...
- **Smart Plot/Report Skips & Parallelization**: Both report generators (`generate_report.py`, `generate_rolling_report.py`) skip rendering diagnostic plots if the image file already exists and is newer than the corresponding model/scaler files. `generate_rolling_report.py` parallelizes plot rendering via `joblib.Parallel(backend="loky")` and precomputes model predictions once (eliminating triple-load redundancy across evaluate/plot/daily-returns). `-j` flag controls workers (0=auto=cpu_count-1). Full 242-plot run: ~19s parallel vs ~3-5min sequential; incremental run: ~7s.
