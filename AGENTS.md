...
- **Daytrade v4 (Walk-Forward Calibration)**: Replaced the single-split-at-holdout calibrator (which selected thresholds/stop/mode by metrics on the same window it then reported on — hyperparameter snooping bias) with **purged expanding-window yearly walk-forward folds** (2021-2026, train = all prior years, 1-day purge gap). Per-side (threshold, conviction, stop, mode) re-selected per fold using train-window data only; trades stitched across test folds → pooled WF metrics. Total deployed pooled WF Sharpe **+39.96** (was +41.94 single-split — the ~2 point drop is the selection bias removed, not a regression). All 10 ETF×side cells still deployable. See [daytrade/AGENTS.md](file:///home/hallo/Documents/option-longterm/daytrade/AGENTS.md) §3.
- **Daytrade v3 (Gating)**: Big-move gating model (per-side big-up/big-down tail classifier) applied as a post-hoc veto over the daytrade linear score. Three target variants (`two_sided`/`joint3`/`gated`) × three feature selectors (`none`/`stability`/`lgbm`) benchmarked; winner auto-picked per cell by honest walk-forward OOS PR-AUC. Walk-forward pooled Sharpe: gated **+39.96** vs ungated **+30.77** (Δ = **+9.20**); 7/10 cells adopt `+gated`. Gate-only (no daytrade score) totals just +9.08 — confirming the gate is a selectivity filter, not a standalone alpha. See [daytrade/GATING_ONLY_REPORT.md](file:///home/hallo/Documents/option-longterm/daytrade/GATING_ONLY_REPORT.md) and [day-model/gating_model/GATING_REPORT.md](file:///home/hallo/Documents/option-longterm/day-model/gating_model/GATING_REPORT.md).
