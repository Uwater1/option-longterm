...
- **Decoupled Ridge Fallback & Dynamic VIF**: Decoupled Ridge fallback from hardcoded ETFs, basing it purely on condition number ($\kappa > 1e5$). Tightened VIF threshold to `5.0` for `50ETF` (10.0 for others) and extended `ridge_alpha` search upper bound to `10000.0`.
