...
python3 day-model/backtest_simulator.py --etf all --long-thr 70 --short-thr 70 [--type {ETF,Future}] [--option] # Run lightweight look-ahead free OOS backtest (trades options when --option is passed)
```
...
- **Two-Sided CV Folds & Consistency Constraints**: Enforced two-sided CV fold metrics `m1..m6` and standard kill-switches. For `side != "single"`, added two side-specific constraints (`side_m2 > 0` Yearly Tail IC Mean, `side_m3 >= 50%` Hit Rate) to prevent fold-level tail sign-flips.
- **Unified Continuous Manifold & Soft Collinearity Penalty**: Replaced discrete categorical model selection (`skglm_huber_l1`, `skglm_mcp`, `ridge`) with a unified `GeneralizedLinearEstimator` using Huber datafit and `MCP_plus_L2(alpha*rho, gamma, alpha*(1-rho))` penalty. Regularization is continuously tuned via total budget (`unified_alpha`) and sparse-vs-ridge mix (`unified_rho`), enabling smooth interpolation. Added a graduated soft condition number penalty `cond_penalty = -0.1 * max(0, log(reg_kappa) - log(SAFE_KAPPA))` to steer Optuna away from ill-conditioned regions before hitting the hard prune threshold at `HARD_KAPPA`.
- **Relative Condition Number Guardrails**: Scaled bounds dynamically to the trial's raw active condition number: `SAFE_KAPPA = 40.0 * raw_X_cond` and `HARD_KAPPA = 10.0 * SAFE_KAPPA`. Tightened VIF threshold to `5.0` for `50ETF` and `159915ETF` to prune collinearity at the source. Capped suggested `unified_rho` with L2 floor when design matrix condition exceeds 15.0.
- **Blocked Bootstrap Validation Regularization**: Run $B=100$ monthly blocked validation bootstrap loops to adjust validation Tail IC by subtracting its bootstrap standard deviation, steering Optuna to select robust hyperparameter plateaus.
...
- **CPCV-Bagged Refit & Blending**: Switched from single-shot final model fit to CPCV bagging over all 15 validation folds. Blending parameter `w` is tuned out-of-sample across Combinatorial Purged CV validation paths to combine single refit and bagged model (default True).
