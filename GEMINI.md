...
- **Decoupled Ridge Fallback & Live Conditioning Constraint**: Removed static pre-decision `force_ridge` based on raw X condition number. Instead, the sampler is free to explore sparse solvers (`skglm_mcp` / `skglm_huber_l1`) and Ridge, while checking regularized Gram matrix condition number (`reg_kappa`) per-trial, prunining/rejecting trials if it exceeds `10000.0`. Added SVD-based post-VIF condition check (`run_cond_pruning`) to drop collinear features until raw cond < 100.0. Tightened VIF threshold to `5.0` for `50ETF` (10.0 for others) and extended `ridge_alpha` search upper bound to `10000.0`.
