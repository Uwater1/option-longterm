...
- [x] Wave 6 volume-profile mining (`dig_wave6_candidates.py`, 23 candidates: 7 volume-profile + 5 cum-vol-curve + 11 intraday-path-archetype). 1 gate-passing single-ETF winner: `min_vol_gap_day` (largest zero-volume gap between traded price bins; IC -0.052, IC_CV 1.31, 7Y-jackknife PASS on 588000ETF single). Integrated as `yesterday_min_vol_gap_day` via `extract_day_full_features` + `cols_to_shift` in build_features.py; all 5 ETF parquets rebuilt (259 features). `intraday_extreme_asymmetry_day` gate-passed on 300/50/588000 but REJECTED — exact algebraic duplicate of existing `yesterday_day_close_pos` (`asym = 2*day_close_pos - 1`, corr 1.0); family logged as rejected in mining memory. Remainder: cum-vol-curve shape (only magnitude-sensitive variants passed, shape-sensitive failed jackknife) and intraday path archetypes (folded-path variants lacked cross-time complementarity) — potential revisit if new loss-side or alternative x-axis formulations arise.
