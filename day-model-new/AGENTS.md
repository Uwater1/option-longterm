...
- **FP32 Engine Acceleration & ECDF Alignment**: `recipe_utils.py` dynamic recipe computation (`compute_recipe`), `build_ecdf_grid_float32`, and Numba factor calculation kernels execute in single-precision FP32 (`float32`), reducing memory bandwidth and maximizing SIMD vector unit execution. `expanding_factor_ic_numba` uses $O(TN)$ online running sum updates instead of $O(T^2 N)$ re-slicing.
