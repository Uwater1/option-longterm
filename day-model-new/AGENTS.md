...
- **Raw vs Cost Sharpe Reporting**: `simulate_returns` computes both raw Sharpe (pre-cost) and cost-adjusted Sharpe (with 15 bps friction per active day). `compile_report.py` displays both side-by-side to distinguish signal quality from transaction cost drag.
