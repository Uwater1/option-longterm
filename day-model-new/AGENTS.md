...
| 9 | Adaptive Boundary Gate | max_pool_size=35, tight_theta=0.75 | Prune weak tail features when pool > 35 |
...
- **Adaptive Boundary Gate (Step 9)**: When initial admission yields $> 35$ features, dynamically tightens training-only quality floors ($S_{train} = 0.40 \times \text{deflated\_ic} + 0.25 \times \text{sortino} + 0.20 \times \text{ic\_ir} + 0.15 \times \text{recent\_ic}$) and correlation threshold $\theta$ ($0.70 \to 0.75$) on lower-tier features to prune weak/decaying/redundant tail features while protecting top TP features. Zero OOS/lockbox data used.
